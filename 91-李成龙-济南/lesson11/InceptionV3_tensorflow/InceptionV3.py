...
warnings.filterwarnings('ignore', category=FutureWarning)  # 暂时忽略未来警告
import numpy as np
...
def InceptionV3(input_shape=[299, 299, 3],
    ...
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)  # 与PPT里InceptionV3原模型有区别
...
def preprocess_input(x):
    ...
    x -= 0.5
    x *= 2.
    return x  # (归一化输入到[-1，1])  #归一化到[0--1]会怎么样？
